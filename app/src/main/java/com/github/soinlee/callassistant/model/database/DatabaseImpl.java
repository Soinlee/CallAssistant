...
public class DatabaseImpl implements Database {
    ...
    public void removeCaller(Caller caller) {
        // Guard against null caller (e.g. no CALLER row exists for the number)
        // to avoid NPE in mDataStore.delete(null) during mark-clear fallback.
        if (caller == null) {
            return;
        }
    ...
    @Override
    public void insertOrUpdateInCall(final InCall inCall) {
        Observable.just(inCall).observeOn(Schedulers.io()).subscribe(new Consumer<InCall>() {
            @Override
            public void accept(InCall inCall) {
                // Idempotent by CALL_LOG_ID: update the existing row when present,
                // insert a new row otherwise. A number may legitimately appear in
                // many rows (one per call), so there is no per-number dedup here.
                InCall existing = mDataStore.select(InCall.class)
                        .where(InCall.CALL_LOG_ID.eq(inCall.getCallLogId()))
                        .get()
                        .firstOrNull();
                if (existing != null) {
                    existing.setNumber(inCall.getNumber());
                    existing.setTime(inCall.getTime());
                    existing.setRingTime(inCall.getRingTime());
                    existing.setDuration(inCall.getDuration());
                    existing.setCallType(inCall.getCallType());
                    existing.setContactId(inCall.getContactId());
                    mDataStore.update(existing);
                } else {
                    mDataStore.insert(inCall);
                }
            }
    ...
    @Override
    public long getMaxCallLogId() {
        InCall last = mDataStore.select(InCall.class)
                .orderBy(InCall.CALL_LOG_ID.desc())
                .get()
                .firstOrNull();
        return last == null ? 0 : last.getCallLogId();
    }

    @Override
    public List<InCall> fetchInCallsSince(long sinceId) {
        return mDataStore.select(InCall.class)
                .where(InCall.CALL_LOG_ID.gt(sinceId))
                .orderBy(InCall.CALL_LOG_ID.desc())
                .get()
                .toList();
    }
    ...
    @Override
    public void removeMarkedRecordSync(String number) {
        // Synchronously delete the marked record and its persisted CALLER row on
        // the calling thread (clearMark runs this on an IO scheduler), so the DB
        // is guaranteed clean before the unmarked resolution is cached. No-op
        // when the marked record does not exist.
        mDataStore.delete(MarkedRecord.class)
                .where(MarkedRecord.NUMBER.eq(number))
                .get().value();
        Caller caller = findCallerSync(number);
        if (caller != null) {
            mDataStore.delete(caller);
        }
    }
